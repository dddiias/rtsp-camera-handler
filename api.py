...
try:
    ...
    print(f"[API] WARNING: python-dotenv not installed, using system env vars only")
...
# ANPR движок больше не нужен - распознавание номеров делает Gemini
...
ENABLE_HIKVISION_API = os.getenv("ENABLE_HIKVISION_API", "false").lower() == "true"  # Старая логика HTTP событий
...
@app.get("/stream/status", summary="Stream Processor Status")
def stream_status() -> Dict[str, Any]:
    """Возвращает статус обработчика потоков"""
    ...
    return {
        "status": "initialized",
        "running": processor._processing_thread is not None and processor._processing_thread.is_alive(),
        "snow_buffer_size": len(processor._snow_frame_buffer),
    }


@app.post("/stream/start", summary="Start Stream Processor")
def stream_start() -> Dict[str, str]:
    """Запускает обработчик потоков"""
    from stream_processor import get_stream_processor
    processor = get_stream_processor()
    
    if processor is None:
        return {"status": "error", "message": "Stream processor not initialized"}
    
    if processor._processing_thread is not None and processor._processing_thread.is_alive():
...
@app.post("/stream/stop", summary="Stop Stream Processor")
def stream_stop() -> Dict[str, str]:
    """Останавливает обработчик потоков"""
    from stream_processor import get_stream_processor
    processor = get_stream_processor()
...
async def start_background_workers():
    ...
    # ============================================
    # Основной режим: обработка RTSP потоков
    # ============================================
    ENABLE_STREAM_PROCESSOR = os.getenv("ENABLE_STREAM_PROCESSOR", "true").lower() == "true"
    ...
    if ENABLE_STREAM_PROCESSOR:
        try:
            ...
            stream_proc.start()
            print("[STARTUP] ✅ Stream processor enabled and started (RTSP mode)")
            print("[STARTUP]    - Plate camera: RTSP stream")
            print("[STARTUP]    - Snow camera: RTSP stream")
        except Exception as e:
            print(f"[STARTUP] ❌ ERROR: Failed to start stream processor: {e}")
            import traceback
            print(f"[STARTUP] Traceback: {traceback.format_exc()}")
    else:
        print("[STARTUP] ⚠️ Stream processor disabled (set ENABLE_STREAM_PROCESSOR=true to enable)")
    
    # ============================================
    # Старая логика (отключена по умолчанию)
    # ============================================
    if ENABLE_HIKVISION_API:
        print("[STARTUP] ⚠️ Legacy Hikvision HTTP API enabled (deprecated)")
    else:
        print("[STARTUP] ✅ Legacy Hikvision HTTP API disabled (using RTSP streams)")
    
    if ENABLE_SNOW_WORKER:
        from snow_worker import start_snow_worker, SNOW_VIDEO_SOURCE_URL
        print(f"[STARTUP] ⚠️ Legacy snow worker enabled (deprecated), source={SNOW_VIDEO_SOURCE_URL}")
        start_snow_worker(UPSTREAM_URL)
    else:
        print("[STARTUP] ✅ Legacy snow worker disabled (using RTSP streams instead)")


@app.post("/anpr", summary="Recognize Plate (Deprecated)")
async def recognize_plate_anpr(
    ...
    Распознавание номеров теперь выполняется через Gemini в stream_processor.
    """
    return JSONResponse(
        {
            "error": "This endpoint is deprecated. Use RTSP stream processing instead.",
            "plate": None,
            "det_conf": 0.0,
            "ocr_conf": 0.0,
            "bbox": None
...
async def choose_plate(camera_plate: str | None, model_plate: str | None) -> tuple[Optional[str], str]:
    """
    DEPRECATED: Эта функция больше не используется в RTSP режиме.
    Распознавание номеров теперь выполняется через Gemini.
    """
    # Просто возвращаем номер от камеры или модели без нормализации
    cam_norm = camera_plate.strip().upper() if camera_plate else None
    model_norm = model_plate.strip().upper() if model_plate else None
...
async def hikvision_isapi(request: Request):
    """
    Старый endpoint для получения событий от камеры через HTTP.
    Отключен по умолчанию (ENABLE_HIKVISION_API=false).
    Используйте RTSP потоки вместо этого.
    """
    if not ENABLE_HIKVISION_API:
        return JSONResponse(
            {"status": "disabled", "message": "Hikvision HTTP API is disabled. Use RTSP streams instead."},
            status_code=503
        )
            ...
            if hasattr(value, "filename"):
                ...
                if ftype.startswith("image/"):
                    ...
                    if lower_name == "detectionpicture.jpg":
                        ...
                        # ANPR больше не используется - распознавание через Gemini
                        model_plate = None
                        model_det_conf = None
                        model_ocr_conf = None
                        model_bbox = None
    ...
    # ANPR больше не используется - этот endpoint отключен по умолчанию
    # Если endpoint включен, просто возвращаем OK (распознавание через Gemini в RTSP режиме)
    model_plate = None
    model_det_conf = None
    model_ocr_conf = None
    model_bbox = None
